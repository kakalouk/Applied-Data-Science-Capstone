...
app.layout = html.Div(children=[html.H1('SpaceX Launch Records Dashboard',
                                ...
                                dcc.Dropdown(id='site-dropdown',
                                             options = [{'label': i, 'value': i} for i in spacex_df['Launch Site'].unique()] + [{'label': 'All Sites', 'value': 'ALL'}],
                                             value='ALL',
...
def get_pie_chart(entered_site):
    ...
    if entered_site == 'ALL':
        ...
        figure = px.pie(class_counts, 
                values=class_counts.values, 
                names=class_counts.index, 
                title='Total successful launches from the selected site')
        return figure
...
def get_scatter_chart(entered_site, entered_payload):
    ...
    if entered_site == 'ALL':
        ...
        figure=px.scatter(data, 
                       ...
                       y='class',
                       color="Booster Version Category",
                       title="Correlation between payload and launch outcome for the selected site")
        return figure
